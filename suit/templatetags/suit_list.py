...
def headers_handler(result_headers, cl):
    ...
    for i, header in enumerate(result_headers):
        ...
        if not isinstance(attrs, dict):
            raise TypeError(
                '"suit_column_attributes" method must return dict. ' "Got: %s: %s" % (attrs.__class__.__name__, attrs)
            )
...
def cells_handler(results, cl):
    ...
    for row, result in enumerate(results):
        ...
        for col, item in enumerate(result):
            ...
            if not isinstance(attrs, dict):
                raise TypeError(
                    '"suit_cell_attributes" method must return dict. ' "Got: %s: %s" % (attrs.__class__.__name__, attrs)
                )
...
def dict_to_attrs(attrs):
    return mark_safe(" " + " ".join(['%s="%s"' % (k, v) for k, v in attrs.items()]))
